...
const branchModel = {
    ...
    createBranch: async (data, connection) => {
        let qry = `
            INSERT INTO branch_master SET
                ...
                branch_status = 1
        `;
        let val = [data.state_id, data.district_id, data.branch_code, data.branch_name];
        let query = connection.format(qry, val);
        return connection.query(query)
            .then(([result]) => {
                return { error: null, result };
    ...
    updateBranch: async (data, connection) => {
        let qry = `
            ...
            WHERE branch_id = ?
        `;
        let val = [data.state_id, data.district_id, data.branch_code, data.branch_name, data.branch_status, data.branch_id];
        let query = connection.format(qry, val);
        return connection.query(query)
            .then(([result]) => {
                return { error: null, result };
